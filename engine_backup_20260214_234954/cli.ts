...
function handleScaffold(specPath?: string): void {
    ...
    log('●', `Scaffolding ${spec.appName} (${specSlug(spec)})...`);
...
function handleValidate(specPath?: string): void {
    ...
    log('●', `Validating spec: ${specSlug(spec)}...`);
...
function handlePatch(specPath?: string): void {
    ...
    log('●', `Generating patches for ${specSlug(spec)}...`);
...
function handleReport(specPath?: string): void {
    ...
    log('●', `Generating report for ${specSlug(spec)}...`);

    const outputValidation = validateOutput(specSlug(spec));
...
function handleStatus(): void {
    ...
    for (const file of specFiles) {
        ...
        try {
            ...
            console.log(`| ${icon} ${spec.status} | ${specSlug(spec)} | ${spec.appName} | ${specPort(spec)} |`);
        } catch {
...
async function handleBuild(specPath?: string): Promise<void> {
    ...
    console.log(`🏭 Full Build Pipeline: ${spec.appName}`);
    ...
    if (!targetDir) {
        targetDir = resolve(PATHS.output, specSlug(spec));
        log('→', `No active project — writing to output/${specSlug(spec)}/`);
    }
    console.log('');
    ...
    const outputValidation = validateOutput(specSlug(spec), outputDir);
    const combined = {
        passed: outputValidation.passed && specValidation.passed,
        checks: [...specValidation.checks, ...outputValidation.checks],
    };
    ...
    console.log('═'.repeat(50));
    if (combined.passed) {
        log('✓', `BUILD COMPLETE: ${spec.appName}`);
    } else {
        log('!', `BUILD COMPLETE WITH WARNINGS: ${spec.appName}`);
    }
    log('→', `Output: ${outputDir}`);
    if (hasTemplate) {
        log('→', `Patches: ${outputDir}/patches/`);
    }
    log('→', `Report: reports/${specSlug(spec)}-*.md`);
    console.log('');
}
